...
#         # Embedding layers for app IDs
#         self.app_embeddings = nn.Embedding(vocab_size, d_model)

#         # Duration projection for time, conditionally included
#         self.duration_projection = (
#             nn.Linear(1, d_model)
...
#         # Calculating the size of the final concatenated embedding
#         feature_count = 1  # Start with App Embeddings only
#         if self.duration_projection:  # Add duration if enabled in config
#             feature_count += 1
#         if self.mouse_clicks_projection:
#             feature_count += 1
#         if self.mouse_scroll_projection:
...
#     def forward(self, app_ids, attention_mask, output_attentions, **kwargs):
#         # Get app embeddings
#         x_apps = self.app_embeddings(app_ids)  # [batch_size, seq_len, d_model]
...
#         # # Handle durations if enabled and provided in kwargs
#         # if self.duration_projection and "durations" in kwargs:
#         #     x_durations = self.duration_projection(
#         #         kwargs["durations"].unsqueeze(-1)
#         #     )  # [batch_size, seq_len, d_model]
#         #     features.append(x_durations)

#         if self.duration_projection and "time" in kwargs:
#             x_time = self.duration_projection(
#                 kwargs["time"].unsqueeze(-1)
#             )  # [batch_size, seq_len, d_model]
#             features.append(x_time)

#         # Add mouseClicks projection if specified in cfg and provided in kwargs
#         if self.mouse_clicks_projection and "mouseClicks" in kwargs:
#             x_mouse_clicks = self.mouse_clicks_projection(
#                 kwargs["mouseClicks"].unsqueeze(-1)
#             )  # [batch_size, seq_len, d_model]
#             features.append(x_mouse_clicks)
...
#         # Add mouseScroll projection if specified in cfg and provided in kwargs
#         if self.mouse_scroll_projection and "mouseScroll" in kwargs:
#             x_mouse_scroll = self.mouse_scroll_projection(
#                 kwargs["mouseScroll"].unsqueeze(-1)
#             )  # [batch_size, seq_len, d_model]
#             features.append(x_mouse_scroll)
...
#         # Add keystrokes projection if specified in cfg and provided in kwargs
#         if self.keystrokes_projection and "keystrokes" in kwargs:
#             x_keystrokes = self.keystrokes_projection(
#                 kwargs["keystrokes"].unsqueeze(-1)
#             )  # [batch_size, seq_len, d_model]
#             features.append(x_keystrokes)
...
#         # Add mic projection if specified in cfg and provided in kwargs
#         if self.mic_projection and "mic" in kwargs:
#             x_mic = self.mic_projection(
#                 kwargs["mic"].unsqueeze(-1)
#             )  # [batch_size, seq_len, d_model]
#             features.append(x_mic)
...
#         # Add camera projection if specified in cfg and provided in kwargs
#         if self.camera_projection and "camera" in kwargs:
#             x_camera = self.camera_projection(
#                 kwargs["camera"].unsqueeze(-1)
#             )  # [batch_size, seq_len, d_model]
#             features.append(x_camera)
...
#         # Add app_quality projection if specified in cfg and provided in kwargs
#         if self.app_quality_projection and "app_quality" in kwargs:
#             x_app_quality = self.app_quality_projection(
#                 kwargs["app_quality"].unsqueeze(-1)
#             )  # [batch_size, seq_len, d_model]
#             features.append(x_app_quality)
...
class ShallowTransformerTimeWithAttention(nn.Module):
    def __init__(
        ...
        # Duration projection for time, conditionally included
        self.duration_projection = (
            nn.Linear(1, d_model) if self.extra_inputs.get("time", False) else None
        )
    ...
    def forward(self, app_ids, attention_mask, output_attentions=False, **kwargs):
        # Get app embeddings
        x_apps = self.app_embeddings(app_ids)  # [batch_size, seq_len, d_model]
        ...
        attention_weights = []  # Store attention weights if required

        # Pass through transformer layers
        for layer in self.encoder_layers:
            if output_attentions:
                # Collect attention weights
                attn_output, attn_weights = layer(
                    x, src_key_padding_mask=attention_mask, need_weights=True
                )
                attention_weights.append(attn_weights)
                x = attn_output
            else:
